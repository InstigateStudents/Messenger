...
private:
	/**
	* This function is automatically called when a server instance is created. It is used to start the update_file and update_online_users threads, manage communications with the client and call the receive command function when a command is given.
	*/
	void run(); 
public:
	/**
	*Sends the list of all online users to all online users once.
	*@param A pointer to a main_server object who's map is used to get the online users.
	*@param The string that must be sent to all online clients. It contains all the online users and their addresses in correct format(It can be taken from parse).
	*/
	static void send_to(int, const std::string&);
	/**
	*Checks if the username and password match. If they do it marks that person as online, and stores his address in the map.
	...
	*/
	...
	/**
	*Checks if the username is taken and creates a new record of a person with the given username and password.
	*@param A pointer to a main_server object who's map is going to be changed.
	...
	*/
	...
	/**
	*Reads the client's input, calls the appropriate command function depending on the string received from the client.
	*@param A pointer to a main_server object who's map is going to be used in later functions.
	*@param Client's socket.
	*@param IP address of the client.
	*/
	static void recieve_command(main_server*, int, const std::string);
	/**
	*Turns a map of users into a string of online users and their addresses that can be transfered to the client.
	*@param The original map
	*/
	std::string parse(std::map<std::string, user_info> mp);
	/**
	* Updates the database every 5 seconds
	*@param A pointer to a main_server object who's map is going to be used to update.
	*/
	static void update_file(main_server*);
	/**
	*Updates the list of online users and sends the parsed map to all online users once every second. 
