...
Main_Client::Main_Client(std::string serv_ip) {
    me.current_socket = socket(AF_INET, SOCK_STREAM, 0);
    me.current_ip = std::to_string(INADDR_ANY);
    if(me.current_socket < 0) {
        std::cerr << "Error in creating socket" << std::endl;
        //exception
    }
    std::thread(connect_server, me.current_socket, serv_ip).detach();

}

void Main_Client::connect_server(int main_socket, std::string serv_ip) {
    ...
    if(connect(main_socket, (sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
